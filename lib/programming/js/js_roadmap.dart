...
class JsRoadMap extends StatefulWidget {
  const JsRoadMap({Key? key}) : super(key: key);

  @override
  State<JsRoadMap> createState() => _JsRoadMapState();
}

class _JsRoadMapState extends State<JsRoadMap> {
  String? localPath;
  ...
  Future<void> downloadPdf() async {
    final taskId = await FlutterDownloader.enqueue(
      ...
      fileName: 'js_roadmap.pdf',
    );

    FlutterDownloader.registerCallback((id, status, progress) {
      if (taskId == id && status == DownloadTaskStatus.complete) {
        setState(() {
          localPath = '/storage/emulated/0/Download/js_roadmap.pdf';
        });
      }
    });
  }
          ...
          Padding(
            ...
            child: IconButton(
              onPressed: () async {
                ...
                String fullPath = "$path/js_roadmap.pdf";
